 ...
 * Layout: Two-column — 3D canvas (left) + collapsible scoring sidebar (right)
 */
...
export default function Home() {
  ...
  const sidebarW = sidebarOpen ? "w-[280px]" : "w-0";
    ...
    <div className="flex w-screen h-screen overflow-hidden bg-background">
      ...
      <div className="relative flex-1 min-w-0 h-full">
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10">
          <HudPanel className="relative">
            <div className="flex items-center gap-3">
              ...
              <div className="w-px h-7 bg-cyan-glow/20" />
        ...
        <div className="absolute bottom-3 left-3 z-10">
          <HudPanel title="Robot Telemetry" className="relative min-w-[220px]">
            <div className="flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-3 gap-2">
                <DataReadout label="X" value={sceneState.robot.position.x} unit="m" color="cyan" />
                <DataReadout label="Y" value={sceneState.robot.position.y} unit="m" color="amber" />
                <DataReadout label="Z" value={sceneState.robot.position.z} unit="m" color="cyan" />
              </div>
              <div className="w-full h-px bg-cyan-glow/10 my-0.5" />
              <div className="flex gap-4">
                <DataReadout label="HDG" value={sceneState.robot.heading} unit="°" color="amber" />
                <DataReadout label="SPD" value={sceneState.robot.speed} unit="m/s" color="green" />
              </div>
            </div>
          </HudPanel>
        </div>
        ...
        {!sidebarOpen && (
          <button
            ...
            className="absolute top-3 right-3 z-10 flex items-center gap-1.5 px-2.5 py-1.5 rounded
                       bg-hud-bg/90 border border-hud-border text-cyan-glow text-[10px] font-medium tracking-wider uppercase
                       hover:bg-cyan-glow/10 hover:border-cyan-glow/40 transition-all duration-200 backdrop-blur-sm"
            title="Show scoring panel"
          >
      ...
      <div
        className={`${sidebarW} flex-shrink-0 h-full transition-all duration-300 ease-in-out overflow-hidden
                    border-l border-hud-border/50 bg-background/95 backdrop-blur-sm`}
      >
        <div className="w-[280px] h-full flex flex-col">
          {/* Sidebar header */}
          <div className="flex items-center justify-between px-3 py-2.5 border-b border-hud-border/30">
            <div className="flex items-center gap-2">
              <Target className="w-3.5 h-3.5 text-cyan-glow" />
              <span className="data-readout text-[11px] text-cyan-glow font-bold tracking-wider uppercase">
                Mission Scoring
              </span>
            </div>
            <button
          ...
          {/* Performance row */}
          <div className="flex items-center gap-4 px-3 py-2 border-b border-hud-border/20">
            <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] text-muted-foreground">Babylon + Rapier</span>
            </div>
          </div>
            ...
            {match.missions.map((m) => {
              ...
              return (
                <div key={m.missionId} className={`rounded transition-colors duration-200 ${
                  ...
                  {/* Mission header — clickable to expand/collapse */}
                  <button
                  ...
                  >
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      <span className={`data-readout text-[11px] font-bold flex-shrink-0 ${allComplete ? "text-green-400" : "text-cyan-glow"}`}>
                        {m.missionId}
                      </span>
                      <span className={`text-[10px] font-medium truncate ${allComplete ? "text-green-400/80" : "text-foreground/80"}`}>
                        {m.missionName}
                      </span>
                    </div>
                  ...
                  {isExpanded && m.conditions.length > 0 && (
                    <div className="px-2 pb-2 ml-5 space-y-1 animate-in slide-in-from-top-1 duration-150">
                      {m.conditions.map((c, ci) => (
                        <div key={ci} className="flex flex-col gap-0.5">
                          <div className="flex items-center justify-between gap-1">
                            ...
                            <span className={`data-readout text-[9px] font-bold flex-shrink-0 ${
          ...
          {/* Sidebar footer: total score */}
          <div className="flex items-center justify-between px-3 py-2.5 border-t border-hud-border/30">
